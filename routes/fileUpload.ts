...
function handleZipFileUpload ({ file }: Request, res: Response, next: NextFunction) {
  if (utils.endsWith(file?.originalname.toLowerCase(), '.zip')) {
    if (((file?.buffer) != null) && utils.isChallengeEnabled(challenges.fileWriteChallenge)) {
      ...
      // 1. Define the absolute path for the intended destination directory.
      const intendedDir = path.resolve('uploads/complaints/')
          ...
          fs.close(fd, function () {
            fs.createReadStream(tempFile)
              ...
              .on('entry', function (entry: any) {
                const unsafeFileName = entry.path
                // 2. Resolve the final destination path securely. This handles '..' characters.
                const destinationPath = path.resolve(intendedDir, unsafeFileName)

                challengeUtils.solveIf(challenges.fileWriteChallenge, () => { return destinationPath === path.resolve('ftp/legal.md') })
                
                // 3. The new, robust security check.
                // It ensures the final path starts with the intended directory path.
                if (destinationPath.startsWith(intendedDir + path.sep)) {
                  // 4. CRITICAL: Use the sanitized 'destinationPath' for writing the file.
                  entry.pipe(fs.createWriteStream(destinationPath).on('error', function (err) { next(err) }))
                } else {
                  // The path is malicious, so we drain it to prevent writing.
                  entry.autodrain()
                }
